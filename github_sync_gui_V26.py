...
APP_TITLE = "GitHub Controlled Folder Sync • Version 26"
...
class GitHubSyncApp(BaseTk):
    ...
    def _build_ui(self) -> None:
        ...
        main = tk.PanedWindow(self, orient="horizontal", sashwidth=7, sashrelief="raised", bg=COLOR_BORDER)
        ...
        right = tk.PanedWindow(main, orient="vertical", sashwidth=7, sashrelief="raised", bg=COLOR_BORDER)
        main.add(left_shell, minsize=330, stretch="always")
        main.add(right, minsize=650, stretch="always")
        ...
        for text,var in [("Different",self.filter_different_var),("Local only",self.filter_local_only_var),("GitHub only",self.filter_remote_only_var),("Both&Same",self.filter_unchanged_var)]:
            ttk.Checkbutton(filters,text=text,variable=var,command=self._apply_compare_filters).pack(side="left",padx=(0,10))
        columns=("location","policy","status","path","size","local_modified")
        self.tree=ttk.Treeview(compare_frame,columns=columns,show="headings")
        for col,text in [("location","Actual location"),("policy","Policy"),("status","Action"),("path","Relative path"),("size","Size"),("local_modified","Modified")]:
            self.tree.heading(col,text=text,command=lambda c=col:self._sort_compare_by(c))
        self.tree.column("location",width=120,stretch=False,anchor="center"); self.tree.column("policy",width=120,stretch=False,anchor="center"); self.tree.column("status",width=210,stretch=False,anchor="center"); self.tree.column("path",width=600,stretch=True); self.tree.column("size",width=105,stretch=False,anchor="e"); self.tree.column("local_modified",width=155,stretch=False,anchor="center")
        ...
        right.add(compare_frame, minsize=240, stretch="always"); right.add(log_frame, minsize=220, stretch="always")
        ...
        def set_sashes(attempt: int = 0):
            # V23: wait until Tk has mapped and measured both PanedWindows.
            # tk.PanedWindow + explicit minsize prevents either pane from
            # collapsing to zero during startup on Windows.
            try:
                ...
                if mw < 900 or rh < 450:
                    if attempt < 12:
                        self.after(100, lambda: set_sashes(attempt + 1))
                    return
                x = max(330, min(int(mw * 0.30), mw - 650))
                y = max(240, min(int(rh * 0.50), rh - 220))
                main.sash_place(0, x, 1)
                right.sash_place(0, 1, y)
            except (tk.TclError, AttributeError):
                if attempt < 12:
                    self.after(100, lambda: set_sashes(attempt + 1))
        self.after_idle(set_sashes)
    ...
    def _authenticated_github_identity(self) -> tuple[Optional[str], Optional[str]]:
        ...
        headers = {
            ...
            "User-Agent": "GitHub-Controlled-Folder-Sync-V26",
        ...
        def task() -> None:
            ...
            for rel in all_paths:
                ...
                rows.append(CompareRow(
                    ...
                    base_status=status,
                    location=("Local" if status == "LOCAL ONLY" else "GitHub" if status == "REMOTE ONLY" else "Both"),
    ...
    def _apply_compare_filters(self) -> None:
        ...
        for row in self._all_compare_rows:
            ...
            # Filters describe the PHYSICAL state discovered by Fetch (Preview),
            # independently of the planned policy/action. A pending action must not
            # force a row into view under the wrong physical-state filter.
        ...
        for row in visible:
            base=row.base_status
            self.tree.insert("","end",values=(row.location,row.policy,row.status,row.path,row.size,row.local_modified),tags=(base,))
    ...
    def _sorted_compare_rows(self, rows: list[CompareRow]) -> list[CompareRow]:
        ...
        elif col=="location": key=lambda r: r.location.lower()
    ...
    def _set_policy(self, paths: Iterable[str], policy: str) -> None:
        ...
        # Right-click changes only the plan, not the physical Fetch snapshot.
        # Recompute the pending-action count and repaint immediately from the same
        # in-memory snapshot; do NOT Fetch and do NOT touch either filesystem.
        self._refresh_compare_summary_from_model()
        self._apply_compare_filters()

    def _refresh_compare_summary_from_model(self) -> None:
        counts = {"LOCAL ONLY": 0, "REMOTE ONLY": 0, "DIFFERENT": 0, "IDENTICAL": 0}
        for row in self._all_compare_rows:
            if row.base_status in counts:
                counts[row.base_status] += 1
        summary = " | ".join(f"{k}: {v}" for k, v in counts.items())
        pending = sum(1 for row in self._all_compare_rows if row.status != "NONE")
        if pending:
            text = summary + f" | PENDING POLICY ACTIONS: {pending}"
        elif counts["LOCAL ONLY"] == 0 and counts["REMOTE ONLY"] == 0 and counts["DIFFERENT"] == 0:
            text = f"LOCAL AND GITHUB MATCH — {counts['IDENTICAL']} unchanged file(s); 0 new; 0 different; 0 missing."
        else:
            text = summary
        self.compare_summary_var.set(text)
    ...
    def _selected_status_paths(self) -> list[tuple[str, str]]:
        """Return (physical_status, relative_path) for selected comparison rows.

        V25: the V22 Actual location column shifted the Treeview value indexes.
        V24 still read indexes 1/2 (Policy/Action), so right-click policies were
        accidentally saved under strings such as NONE instead of the file path.
        Resolve the selected row back to the in-memory comparison model by its
        Relative path column (index 3), keeping the Fetch snapshot authoritative.
        """
        out: list[tuple[str, str]] = []
        rows_by_path = {row.path.replace("\\", "/"): row for row in self._all_compare_rows}
        for iid in self.tree.selection():
            vals = self.tree.item(iid, "values")
            if len(vals) < 4:
                continue
            rel = str(vals[3]).replace("\\", "/")
            row = rows_by_path.get(rel)
            if row is not None:
                out.append((row.base_status, row.path))
        return out
    ...
    def _confirm_synchronize_plan(self, plan: dict[str, list[str]]) -> bool:
        """Modal V24 synchronization-plan summary. No discovery or file operation occurs here."""
        ...
        counts.pack(fill="x", pady=(0, 8))
        count_items = [
            ("To GitHub", len(set(plan["upload"]) | set(plan["overwrite_github"])), False),
            ("To Local", len(plan["download"]), False),
            ("DEL LOCAL", len(plan["delete_local"]), True),
            ("DEL GITHUB", len(plan["delete_github"]), False),
        ]
        for col, (label, count, danger) in enumerate(count_items):
            counts.grid_columnconfigure(col, weight=1)
            tk.Label(
                counts, text=f"{label}:  {count}",
                fg=COLOR_RED if danger and count else COLOR_TEXT,
                bg=COLOR_BG,
                font=("Segoe UI", 11, "bold") if danger else ("Segoe UI", 10, "bold"),
                anchor="center",
            ).grid(row=0, column=col, sticky="ew", padx=4)

        # Keep the overwrite count explicit because an overwrite and a new upload
        # both appear in the To GitHub column below.
        ttk.Label(
            outer,
            text=(f"New uploads: {len(plan['upload'])}     "
                  f"GitHub overwrites (PC wins): {len(plan['overwrite_github'])}"),
        ).pack(anchor="w", pady=(0, 6))

        ttk.Separator(outer).pack(fill="x", pady=(2, 8))

        # V26: four independent list columns with one shared vertical scrollbar.
        # Unlike ttk.Treeview, Listbox can style the Del Local filenames themselves
        # in bold red without coloring unrelated cells in the same visual row.
        panel = ttk.Frame(outer)
        panel.pack(fill="both", expand=True)
        panel.grid_rowconfigure(1, weight=1)
        for col_index in range(4):
            panel.grid_columnconfigure(col_index, weight=1, uniform="syncplan")

        columns = ("to_github", "to_local", "del_local", "del_github")
        lists = {
            "to_github": sorted(set(plan["upload"]) | set(plan["overwrite_github"]), key=str.lower),
            "to_local": sorted(set(plan["download"]), key=str.lower),
            "del_local": sorted(set(plan["delete_local"]), key=str.lower),
            "del_github": sorted(set(plan["delete_github"]), key=str.lower),
        }
        headings = {
            "to_github": "To GitHub",
            "to_local": "To Local",
            "del_local": "Del Local",
            "del_github": "Del GitHub",
        }
        reverse = {col: False for col in columns}
        listboxes: dict[str, tk.Listbox] = {}

        def sync_yview(first: str, last: str) -> None:
            # A scroll gesture in any column keeps all four action lists aligned.
            for lb in listboxes.values():
                lb.yview_moveto(first)
            plan_scroll.set(first, last)

        def scroll_all(*args: str) -> None:
            for lb in listboxes.values():
                lb.yview(*args)

        def rebuild_plan_column(col: str) -> None:
            lb = listboxes[col]
            lb.delete(0, "end")
            for rel in lists[col]:
                lb.insert("end", rel)

        def sort_action_column(col: str) -> None:
            lists[col].sort(key=str.lower, reverse=reverse[col])
            reverse[col] = not reverse[col]
            rebuild_plan_column(col)

        for idx, col in enumerate(columns):
            danger = col == "del_local"
            heading = tk.Button(
                panel,
                text=headings[col],
                command=lambda c=col: sort_action_column(c),
                bg=COLOR_NAVY,
                fg=COLOR_RED if danger else COLOR_WHITE,
                activebackground=COLOR_NAVY_2,
                activeforeground=COLOR_RED if danger else COLOR_WHITE,
                font=("Segoe UI", 10, "bold"),
                relief="raised",
                bd=1,
            )
            heading.grid(row=0, column=idx, sticky="ew", padx=(0 if idx == 0 else 1, 1), pady=(0, 2))
            ...
            lb = tk.Listbox(
                ...
                bg=COLOR_WHITE,
                fg=COLOR_RED if danger else COLOR_TEXT,
                font=("Segoe UI", 9, "bold") if danger else ("Segoe UI", 9),
                relief="solid",
                bd=1,
                highlightthickness=0,
                xscrollcommand=None,
            )
            lb.grid(row=1, column=idx, sticky="nsew", padx=(0 if idx == 0 else 1, 1))
            listboxes[col] = lb

        plan_scroll = ttk.Scrollbar(panel, orient="vertical", command=scroll_all)
        plan_scroll.grid(row=1, column=4, sticky="ns")
        # One listbox reports the thumb position; all are kept at the same y offset.
        listboxes["to_github"].configure(yscrollcommand=sync_yview)
        for col in columns:
            rebuild_plan_column(col)

        def _wheel_plan(event):
            delta = getattr(event, "delta", 0)
            if delta:
                units = -int(delta / 120) if abs(delta) >= 120 else (-1 if delta > 0 else 1)
                for lb in listboxes.values():
                    lb.yview_scroll(units, "units")
                first, last = listboxes["to_github"].yview()
                plan_scroll.set(first, last)
                return "break"
            return None

        for lb in listboxes.values():
            lb.bind("<MouseWheel>", _wheel_plan)

        ttk.Label(
            outer,
            text="Click a column heading to sort that action's files alphabetically. "
                 "Press Synchronize to execute exactly this plan. The program will revalidate "
                 "the Local/GitHub state first and abort if it changed.",
            wraplength=980,
        ...
        ttk.Button(buttons, text="Cancel", command=win.destroy).grid(
            row=0, column=0, sticky="ew", padx=(0, 5)
        )
        ttk.Button(buttons, text="SYNCHRONIZE", style="Green.TButton", command=accept).grid(
    ...
    def synchronize_all(self) -> None:
        ...
        def task() -> None:
            ...
            # V26 local-deletion safety:
            # - GITHUB ONLY means the remote copy is intentionally being retained, so
            #   GitHub MUST contain the file before its local copy may be removed.
            # - DELETE is an explicit request to remove the file from the requested
            #   side(s). Requiring the file to still exist on GitHub after an approved
            #   GitHub deletion made DELETE-from-both impossible in V25.
            if plan["delete_local"]:
                must_exist_remotely = [
                    rel for rel in plan["delete_local"]
                    if self._policy_for(rel) == "GITHUB_ONLY"
                ]
                if must_exist_remotely:
                    if not rr:
                        raise GitError("GitHub could not be verified, so GITHUB ONLY local removal was aborted.")
                    verified_remote = self._remote_tree(folder, rr)
                    missing = [rel for rel in must_exist_remotely if rel not in verified_remote]
                    if missing:
                        raise GitError(
                            "Safety check failed: these GITHUB ONLY files are not verified on GitHub, "
                            "so their local deletion was aborted:\n\n" + "\n".join(missing[:20])
                        )
                batch_stamp = time.strftime("%Y%m%d_%H%M%S")
            ...
            if rr:
                ...
                self._git("branch", "--set-upstream-to", rr, branch, cwd=folder, check=False)
            self._update_remote_state_worker(folder)
            self.log("V26 synchronization completed successfully.", "OK")
