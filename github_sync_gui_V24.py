...
APP_TITLE = "GitHub Controlled Folder Sync • Version 24"
...
class GitHubSyncApp(BaseTk):
    ...
    def _confirm_synchronize_plan(self, plan: dict[str, list[str]]) -> bool:
        ...
        # V24: four side-by-side action columns.  The Treeview supplies one shared
        # vertical scrollbar for the complete panel.  Each heading independently
        # sorts its own file list alphabetically; repeated clicks toggle A-Z/Z-A.
        ...
        plan_tree = ttk.Treeview(panel, columns=columns, show="headings", selectmode="browse")
        plan_scroll = ttk.Scrollbar(panel, orient="vertical", command=plan_tree.yview)
        plan_tree.configure(yscrollcommand=plan_scroll.set)
        plan_tree.grid(row=0, column=0, sticky="nsew")
        plan_scroll.grid(row=0, column=1, sticky="ns")

        for col in columns:
            plan_tree.column(col, width=235, minwidth=120, stretch=True, anchor="w")
        ...
        # Treeview cannot style only one individual cell.  Deletion danger is
        # therefore kept explicit in the bold red DEL LOCAL count above, while
        # this grid remains easy to scan and sort.
        def rebuild_plan_grid() -> None:
            plan_tree.delete(*plan_tree.get_children())
            longest = max((len(lists[c]) for c in columns), default=0)
            for i in range(longest):
                values = tuple(lists[c][i] if i < len(lists[c]) else "" for c in columns)
                plan_tree.insert("", "end", values=values)
        ...
        for col in columns:
            plan_tree.heading(col, text=headings[col], command=lambda c=col: sort_action_column(c))
