...
Debug_B <- TRUE   
MinimumProfilLength <- 5   # Used to remove analyzis that have less than this number of days analyzed
...
QuantityFirstDaysToRemove = 0 # Quantity of days removed at the begining for complete correlation windows
TimeBining <- 1 # Quantity of days to average to create a data point (default = 1)
TimeWindow <- 10 # The quantity of days that the algorithm will use to search correlations inside
...
ScoreInFunctionOfEpilepsy_B = TRUE     #KEEPS ON TRUE, needs some variables from this part of script !!!!
...
#region#green    Remove first days for total windows correlation
...
Input_L_Raw[which(Input_L_Raw[,6]=="EMPTYTO0",),8]<-MinimumProfilLength*100
...
#region#red #creer EmptyTo0_Scores_L
...
print("Writting files . . . ")
        ...
        temp1<-paste("cor",Correl_Thresold,"_min",MinimumProfilLength,"_Pts",MinimumQuantityOfPatientsPerProfil,"_bin",TimeBining,"_Shft",TimeShift,"_W",TimeWindow,sep="","_R",QuantityFirstDaysToRemove)
