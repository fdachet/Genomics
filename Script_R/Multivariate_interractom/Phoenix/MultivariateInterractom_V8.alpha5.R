...
colnames(Input_A_Interpoled_Matrix)<-paste(Input_A_Interpoled[1,],"|",Input_A_Interpoled[2,],sep="")
...
for (indice_variable in 1:dim(Input_A_Interpoled_Matrix)[2])
{
print(colnames(Input_A_Interpoled_Matrix)[indice_variable])



start_time <- Sys.time()
r10 <- rollapplyr(Input_A_Interpoled_Matrix[,1:10], 5, ccor, fill = NA, by.column = FALSE) #0.02
end_time <- Sys.time()
end_time - start_time


start_time <- Sys.time()
r100 <- rollapplyr(Input_A_Interpoled_Matrix[,1:100], 5, ccor, fill = NA, by.column = FALSE) #2s
end_time <- Sys.time()
end_time - start_time

start_time <- Sys.time()
r200 <- rollapplyr(Input_A_Interpoled_Matrix[,1:200], 5, ccor, fill = NA, by.column = FALSE) #8s
end_time <- Sys.time()
end_time - start_time

start_time <- Sys.time()
r400 <- rollapplyr(Input_A_Interpoled_Matrix[,1:400], 5, ccor, fill = NA, by.column = FALSE) #33s
end_time <- Sys.time()
end_time - start_time
...
start_time <- Sys.time()
r600 <- rollapplyr(Input_A_Interpoled_Matrix[,1:600], 5, ccor, fill = NA, by.column = FALSE) #1m20
end_time <- Sys.time()
end_time - start_time
...
r2 <- rollapplyr(Input_A_Interpoled_Matrix, 5, ccor, fill = NA, by.column = FALSE) 
r2 <- rollapplyr(Input_A_Interpoled_Matrix, 5, ccor, fill = NA, by.column = FALSE) 
r2 <- rollapplyr(Input_A_Interpoled_Matrix, 5, ccor, fill = NA, by.column = FALSE) 
r2 <- rollapplyr(Input_A_Interpoled_Matrix, 5, ccor, fill = NA, by.column = FALSE) 
...
r2 <- rollapplyr(Input_A_Interpoled_Matrix, 5, ccor, fill = NA, by.column = FALSE)  #
r1<-rollapplyr(Data, width=indice_Window, fill = NA, by.column = FALSE)  #

paste_ <- function(...) paste(..., sep = "_")
names_mat <- do.call("outer", list(names(ts), names(ts), paste_))

names(r1) <- names_mat
names(r2) <- names_mat[lower.tri(names_mat)]  # put the names
}
