...
#Input_A_Interpoled_ts<-ts(Input_A_Interpoled_Matrix)
...
Result_Correlation_Vertical<-data.frame(matrix(NA,1,6))      #Patient,Variable1,Variable2,Correlation,longuerProfilDays,Pvalue)
colnames(Result_Correlation_Vertical)<-c("Patient","Variable1","Variable2","Correlation","longuerProfilDays","Pvalue")
...
{
Input_A_Interpoled_Matrix_PerPatient_EC<-Input_A_Interpoled_Matrix[  ,which(Input_A_Interpoled[1,]==Patient_List[indice_patient])  ]
Variable_PerPatient_Quantity<-length(levels(as.factor(as.character(Input_A_Interpoled[2,]))))
Result_Correlation_Horyzontal_PerPatient<-data.frame(matrix(NA,1,2+Variable_PerPatient_Quantity^2-Variable_PerPatient_Quantity))
...
Input_A_Interpoled_Matrix_PerPatient_PerWindow <- Input_A_Interpoled_Matrix[  ,which(Input_A_Interpoled[1,]==Patient_List[indice_patient])  ]




temp_correlation_1 <- rollapplyr(Input_A_Interpoled_Matrix_PerPatient_PerWindow, width=indice_CorrelationWindow , ccor, fill = NA, by.column = FALSE) #0.02
paste_ <- function(...) paste(..., sep = "_|_")
names_mat <- do.call("outer", list(colnames(Input_A_Interpoled_Matrix_PerPatient_PerWindow), colnames(Input_A_Interpoled_Matrix_PerPatient_PerWindow), paste_))

colnames(temp_correlation_1) <- names_mat[lower.tri(names_mat)]  # put the names


temp_correlation_2<-cbind(Patient_List[indice_patient],indice_CorrelationWindows,temp_correlation_1)

Result_Correlation_Horyzontal_PerPatient<-rbind(temp_correlation_2,Result_Correlation_Horyzontal_PerPatient)
