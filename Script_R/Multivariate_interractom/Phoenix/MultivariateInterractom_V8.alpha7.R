...
{
Input_A_Interpoled_Matrix_EC <- Input_A_Interpoled_Matrix[  ,which(Input_A_Interpoled[1,]==Patient_List[indice_patient])  ]




temp_correlation_1 <- rollapplyr(Input_A_Interpoled_Matrix_EC, width=MinimumProfilLength , ccor, fill = NA, by.column = FALSE) #0.02


paste_ <- function(...) paste(..., sep = "_|_")
names_mat <- do.call("outer", list(colnames(Input_A_Interpoled_Matrix_EC), colnames(Input_A_Interpoled_Matrix_EC), paste_))
