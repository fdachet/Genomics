...
Variable_PerPatient_Quantity<-length(levels(as.factor(as.character(Input_A_Interpoled_Matrix_PerPatient_EC[2,]))))
...
temp_correlation_1 <- rollapplyr(Input_A_Interpoled_Matrix_PerPatient_EC, width=indice_CorrelationWindow , ccor, fill = NA, by.column = FALSE) #0.02
...
#select only the best abs correl for this windows()
apply(temp_correlation_1,2,function(X){which(mac(abs(X)))})
