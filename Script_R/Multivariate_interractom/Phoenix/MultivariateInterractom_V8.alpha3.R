...
Input_Raw<-Input_Raw[,-ncol(Input_Raw)]
...
print("Debug")
temp<-apply((Input_A_Interpoled[3:nrow(Input_A_Interpoled),]),2,function(X) { tryCatch ( { na_interpolation(X,maxgap=Interpolation_maxgap_inDays,option="stine")}, error=function(e) {NA} )})
temp<-as.data.frame(temp)
colnames(Input_A_Interpoled)<-colnames(temp)
temp2<-t(Input_A[,1:2])
colnames(temp2)<-colnames(temp)
Input_A_Interpoled<-rbind(temp2,temp)
#Input_A_Interpoled<-t(Input_A_Interpoled)
colnames(Input_A_Interpoled)<-paste(Input_A_Interpoled[1,],"|",Input_A_Interpoled[2,],sep="")
Input_A_Interpoled<-Input_A_Interpoled[c(-1,-2),]
Input_A_Interpoled<-apply(Input_A_Interpoled,2,as.numeric)
      #endregion#
...
for (indice_variable in 1:dim(Input_A_Interpoled)[2])
{
for (indice_Window in MinimumProfilLength:MaximumProfilLength)
{




r2 <- rollapplyr(Input_A_Interpoled, 63, ccor, fill = NA, by.column = FALSE)
r1<-rollapplyr(Data, width=indice_Window, fill = NA, by.column = FALSE)  #garde only the non redundant half matrice
