...
if (isTRUE(cli$help)) {
    visium_print_help(
        ...
        c(
            ...
            "--user-defined-combination-method CSV  One or more: sum,weighted_mean_by_sign,mean_by_sign,weighted_rank_by_sign.", "--user-defined-gene-normalization CSV  One or more: robust_minmax,minmax,none.",
...
list(name="custom_help_norm",type="info",group="Linear Model pathway scoring",text="robust_minmax clips each gene to the 1st and 99th percentiles before rescaling; minmax uses the full observed range; none uses assay values unchanged. Per-gene normalization is applied before gene-score combination. The final 0-to-1 pathway value is a within-pathway, within-tissue relative score: compare locations for the same pathway, not absolute magnitude between different pathways."),
...
list(name="custom_help_weighted_sign",type="info",group="Linear Model pathway scoring",text="weighted_mean_by_sign calculates the weighted mean of positive genes minus the weighted mean of negative genes, using the absolute pathway weights to normalize each sign separately. Relative gene importance is preserved without allowing the total weight of one sign to inflate the pathway score."),
...
list(name="custom_help_weighted_rank",type="info",group="Linear Model pathway scoring",text="weighted_rank_by_sign converts each pathway gene to its 0-to-1 expression percentile rank across spots after the selected per-gene normalization, then calculates the weighted positive mean minus the weighted negative mean. This reduces sensitivity to expression-scale differences and extreme values.")
...
if (!length(USER_DEFINED_COMBINATION_METHODS) || any(!USER_DEFINED_COMBINATION_METHODS %in% c("sum", "weighted_mean_by_sign", "mean_by_sign", "weighted_rank_by_sign"))) stop("Gene-score combination method must contain only sum, weighted_mean_by_sign, mean_by_sign, or weighted_rank_by_sign.")
...
build_user_defined_scores <- function(object,
    ...
    for (i in seq_along(parsed_rows)) {
        ...
        if (combination_method == "sum") {
        ...
        } else if (combination_method == "weighted_mean_by_sign") {
            ...
            pos_weight_sum <- sum(vapply(resolved[pos_terms], function(term) abs(term$weight), numeric(1)))
            neg_weight_sum <- sum(vapply(resolved[neg_terms], function(term) abs(term$weight), numeric(1)))

            pos_score <- if (length(pos_terms) && is.finite(pos_weight_sum) && pos_weight_sum > 0) {
                rowSums(score_matrix[, pos_terms, drop = FALSE]) / pos_weight_sum
            } else {
                rep(0, length(all_cells))
            }

            neg_score <- if (length(neg_terms) && is.finite(neg_weight_sum) && neg_weight_sum > 0) {
                rowSums(abs(score_matrix[, neg_terms, drop = FALSE])) / neg_weight_sum
            } else {
                rep(0, length(all_cells))
            }
            ...
            raw_scores <- pos_score - neg_score
        } else if (combination_method == "weighted_rank_by_sign") {
            ranked_expression <- t(apply(
                expr_after[vapply(resolved, `[[`, character(1), "matched_gene"), , drop = FALSE],
                1L,
                function(v) {
                    finite_mask <- is.finite(v)
                    ...
                    finite_values <- v[finite_mask]
                    if (n_finite == 1L || max(finite_values) <= min(finite_values)) {
                        out[finite_mask] <- 0
                        return(out)
                    }

                    ranked <- rank(finite_values, ties.method = "average")
                    out[finite_mask] <- (ranked - 1) / (n_finite - 1)
                    out
                }
            ))

            rownames(ranked_expression) <- vapply(resolved, `[[`, character(1), "matched_gene")
            colnames(ranked_expression) <- all_cells

            rank_score_matrix <- sapply(
                resolved,
                function(term) {
                    term$sign * term$weight * ranked_expression[term$matched_gene, ]
                }
            )

            if (is.null(dim(rank_score_matrix))) {
                rank_score_matrix <- matrix(rank_score_matrix, nrow = length(all_cells), ncol = 1L)
            }
            ...
            pos_score <- if (length(pos_terms) && is.finite(pos_weight_sum) && pos_weight_sum > 0) {
                rowSums(rank_score_matrix[, pos_terms, drop = FALSE]) / pos_weight_sum
            } else {
                rep(0, length(all_cells))
            }

            neg_score <- if (length(neg_terms) && is.finite(neg_weight_sum) && neg_weight_sum > 0) {
                rowSums(abs(rank_score_matrix[, neg_terms, drop = FALSE])) / neg_weight_sum
            } else {
                rep(0, length(all_cells))
            }
...
if (PLOT_USER_DEFINED_FEATURES) {
    ...
    for (combination_method in USER_DEFINED_COMBINATION_METHODS) {
        for (normalization_method in USER_DEFINED_GENE_NORMALIZATIONS) {
            ...
            # Keep the combination/normalization details in the parent folder name,
            # but use a compact RDS filename to avoid the Windows MAX_PATH limit.
            combination_method_tag <- switch(
                combination_method,
                "sum" = "Sum",
                "mean_by_sign" = "MeanSign",
                "weighted_mean_by_sign" = "WMeanSign",
                "weighted_rank_by_sign" = "WRankSign",
                sanitize_name(combination_method)
            )
            normalization_method_tag <- switch(
                normalization_method,
                "robust_minmax" = "RMinMax",
                "minmax" = "MinMax",
                "none" = "None",
                sanitize_name(normalization_method)
            )
            custom_rds_file <- file.path(
                combination_directory,
                paste0(
                    visium_workspace_sample_name(RESULTS_WORKSPACE),
                    "_S05_", METHOD_OUTPUT_TAG, "_",
                    combination_method_tag, "_",
                    normalization_method_tag, ".rds"
                )
            )
