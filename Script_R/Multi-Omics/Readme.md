...
1. **Prepare the multi-omics table.** Measurements are organised in tabulated text file as features (rows) by samples (columns). Each omics layer is normalised within its own measurement type before correlation, so the network of correlation is not baised by noise. This reduces some inter-patient/technical variability while preserving the between-sample pattern that will be compared.
2. **Turn complex factors into simple variables.** A clinical factor is often not one number. Treatment, sex, disease subgroup, cell type, or medication exposure can be separated into several dummy variables, usually min–max scaled and optionally binominal columns (0/1). This lets individual parts of a complex factor be correlated with genes, proteins, metabolites, and clinical measurements without giving a false numeric order to categories.
3. **Calculate relationships between features.** [`1.Interractom_2025.R`](1.Interractom_2025.R) calculates positive Pearson correlations and can also calculate Spearman and Kendall rank correlations. Pearson asks whether two measurements change together in an approximately linear way and is highly dependent of outlier values while Spearman asks whether their ranks move together and is less affected by unusual scales/outliers. Kendall compares concordant and discordant pairs and can be useful in small cohorts.
4. **Retain the strongest and most supported links.** The user can select the top positive results, set a correlation threshold, or set a P-value threshold to combine different type of correlations. Else when more than one correlation method is enabled, the retained table brings that evidence together for the same feature pair. This prevents the final network being filled with every weak association.
...
| `pvalue_threshold` | Maximum P value allowed when using P-value filtering. A small P value means the observed association has non-random association under the model, not that it is clinically important by itself. |
...
*The same pair of nodes can sometimes have more than one edge in Cytoscape. These multi-edges may represent different correlation methods (Pearson, Spearman, or Kendall), repeated evidence, or separate association records. In this visual style representation, a thicker/darker edge represents a stronger single bond between two nodes—usually based on a larger correlation coefficient or, if the style is configured that way, a smaller associated P value (any type of correlation).

*The positions of nodes come from a spring/repulsion layout. Linked nodes pull toward each other, while all nodes repel enough to prevent the network collapsing into one point. Therefore, nodes with many mutually reinforcing links often appear closer together and form visible neighbourhoods. Two nodes joined by only one weak or isolated edge usually stay more peripheral. This proximity is a visual plausibility cue: a dense group is more interesting than a single link*

## Example: a dense blood-related module

![Detailed blood-cell cluster](Screenshots/Cluster_blood_cells_White_V0.jpg)

*Figure 5. A close-up of a dense module identified in the larger network. The large blue node is the blood-cell feature. Around it are protein nodes of several types (red cytosolic, orange nuclear, and magenta synaptosomal triangles), RNA nodes (gray/yellow circles), metabolites (cyan squares), and the medication Vimp at lower left (green shape). The heavy concentration of edges in the centre means that many features have retained correlations with one another, so the group is more plausible as a coordinated biological/technical module than two nodes connected by a single line. Labels such as HLA II, hemoglobin, α2-macroglobulin, immunoglobulin, fibrinogens, catalases, glycine, biliverdin, and carbonic anhydrases make the module easier to discuss with domain experts. AllegroMCODE can help identify such dense regions automatically, then the researcher can review whether the module suggests a blood/immune signal, a cell-composition effect, medication association, sample handling effect*

## Example: an age-associated module

![Age-associated cluster](Screenshots/Cluster_Age.jpg)

*Figure 6. Close-up of an age-associated local network. The magenta diamond labelled `Age` is a clinical variable. It is connected to several molecular measurements, including the red triangular cytosolic-protein nodes `Gene_SPDYE1`, `CRMP1`, `CRMP5` and `Gene_TMEM45A`, non-coding RNA nodes such as `LNCRNA_BC044856_6239` and `LNCRNA_BC033557_7210`, and the green V-shaped medication node `Vigabatrin`. The orange triangle represents a nuclear protein. Reading the picture from the centre outwards is an easy way to see which features are most directly associated with age; thicker, darker lines are the stronger retained associations and thin grey lines are weaker ones. This mixed cluster is important because age can be linked with biology, disease duration, treatment exposure and clinical subgroups at the same time. It is therefore a useful warning to check or adjust for age when interpreting a disease-related signal*

## A simple interpretation example

Cluster indicates a blood-cell node is linked to HLA II, hemoglobin, immunoglobulin, and several metabolites. It may reflect immune activation, blood contamination, a change in cellular composition, disease severity, treatment, or another clinical factor that moves together across samples. The network helps find the question and then follow-up analysis and clinical knowledge decide what the association may mean.

Dense modules are often more useful than isolated links because several independent-looking measurements reinforce the same idea.

## Cytoscape and AllegroMCODE

1. Run `1.Interractom_2025.R` after preparing `Input.tabtxt` (the inputed tabulated text file containing your multi-omics data).
2. Import the generated `R_Correlation_*.tabtxt` file into Cytoscape V2.8.3 as an edge table, using `GeneSource` as source and `GeneTarget` as target.
...
5. Install/use the archive in [`Plugin_Alegro_Mcode/`](Plugin_Alegro_Mcode/) for AllegroMCODE clustering. 
