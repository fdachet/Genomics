...
calculate_observed_scores <- function(model, normalized_expression, all_cells) {
    ...
    for (p in pathways) {
        pm <- model[model$pathway == p & model$feature_found, , drop = FALSE]
        requested_n <- sum(model$pathway == p)
        if (!nrow(pm)) next
        ...
        score[p, ] <- as.numeric(w %*% x) / denom
        pearson[p, ] <- safe_cor_by_column(w, x, method = "pearson")
        spearman[p, ] <- safe_cor_by_column(w, x, method = "spearman")
        pearson_p[p, ] <- correlation_p_from_r(pearson[p, ], length(w), "Positive fingerprint match (one-sided)")
        spearman_p[p, ] <- correlation_p_from_r(spearman[p, ], length(w), "Positive fingerprint match (one-sided)")

        coverage_rows[[p]] <- data.frame(
            pathway = p,
...
run_pathway_footprint_analysis <- function(settings, model_full, selected_pathways, object = NULL) {
    ...
    observed <- calculate_observed_scores(weighted_model, normalized_selected, all_cells)
    ...
    classical_corr_p <- matrix(NA_real_, nrow = nrow(primary_correlation), ncol = ncol(primary_correlation), dimnames = dimnames(primary_correlation))
    for (p in rownames(primary_correlation)) {
        pm <- weighted_model[weighted_model$pathway == p & weighted_model$feature_found, , drop = FALSE]
        pm <- pm[order(-abs(pm$effective_weight)), , drop = FALSE]
        pm <- pm[!duplicated(pm$matched_feature), , drop = FALSE]
        classical_corr_p[p, ] <- correlation_p_from_r(primary_correlation[p, ], nrow(pm), settings$significance_tail)
    }
    ...
    if (identical(settings$significance_source, "Empirical weight permutation")) {
        ...
        for (i in seq_len(nrow(primary_correlation))) {
            ...
            set_status(paste0("Correlation permutation test: ", p, " (", i, "/", nrow(primary_correlation), ")"), 30 + 35 * i / nrow(primary_correlation))
            append_log("Empirical correlation permutations for pathway ", p, "...")
            pm <- weighted_model[weighted_model$pathway == p, , drop = FALSE]
            pmat[p, ] <- empirical_correlation_p_for_pathway(
                pathway = p,
                pm = pm,
                normalized_expression = normalized_selected,
                observed_correlation = primary_correlation[p, ],
