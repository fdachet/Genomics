...
Partial least-squares discriminant analysis constructs latent components from the predictors and uses those components to distinguish the training classes.
...
- `Best_Tuning_Parameters.tabtxt` contains the selected number of latent components (`ncomp`).
...
`Training_Warnings.txt`, when present, contains warnings captured while fitting the model.
