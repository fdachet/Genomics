# Radial support-vector-machine results

This model uses a radial-basis-function kernel to represent nonlinear class boundaries after median imputation, centering, and scaling.
...
- `Best_Tuning_Parameters.tabtxt` contains the selected radial-kernel width (`sigma`) and cost (`C`).
...
![SVM classification probability heatmap](Classification_Probability_Heatmap.png)

Rows represent samples and columns represent Classes A, B, and C. Darker cells contain larger probabilities. Clear samples form three high-probability diagonal blocks. The ambiguous rows at the bottom distribute probability across A/B or B/C according to their mixed synthetic signal.
